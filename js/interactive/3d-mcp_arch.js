...
(function () {
  ...
  var HOST_COLOR = 0x20B2AA;
  var CLIENT_COLOR = 0x3b82f6;
  var SERVER_COLOR = 0xf59e0b;
  ...
  var NODES = [
    ...
    { id: 'server_0', type: 'server', angle: Math.PI/6,        radius: 3.2 },
    { id: 'server_1', type: 'server', angle: 2*Math.PI/3,      radius: 3.2 },
    { id: 'server_2', type: 'server', angle: 4*Math.PI/5,      radius: 3.2 },
    { id: 'server_3', type: 'server', angle: 13*Math.PI/8,     radius: 3.2 },
  ];

  var EDGES = [
    { from: 'host', to: 'client_0', midOff: [-0.2, 0.0] },
    { from: 'host', to: 'client_1', midOff: [0.15, -0.15] },
    { from: 'host', to: 'client_2', midOff: [-0.1, 0.25] },
    { from: 'host', to: 'client_3', midOff: [0.25, 0.1] },
    { from: 'client_0', to: 'server_0', midOff: [-0.3, 0.35] },
    { from: 'client_1', to: 'server_1', midOff: [0.4, -0.2] },
    { from: 'client_2', to: 'server_2', midOff: [-0.35, -0.4] },
    { from: 'client_3', to: 'server_3', midOff: [0.2, -0.3] },
  ];
  ...
  function nodePos(n) {
    if (n.radius === 0) return new THREE.Vector3(0, 3.0, 0);
    var y = 0.3 * Math.sin(n.angle * 2) + 3.0;
  ...
  McpArch3D.prototype.initScene = function () {
    ...
    this.camera.position.set(2.5, 5.5, 5);
  ...
  McpArch3D.prototype.initNodes = function () {
    ...
    NODES.forEach(function (n) {
      ...
      var lbl = self.makeLabel(n.id.replace('_', ' '), '#' + new THREE.Color(color).getHexString());
      lbl.position.copy(pos);
      lbl.position.y -= radius + 0.3;
  ...
  McpArch3D.prototype.initEdges = function () {
    ...
    EDGES.forEach(function (e, idx) {
      ...
      var color = e.from === 'host' ? HOST_COLOR : SERVER_COLOR;

      var pts = [from.clone(), to.clone()];
      var path = new THREE.CatmullRomCurve3(pts);
      var tubeGeo = new THREE.TubeGeometry(path, 6, 0.012, 5, false);
      ...
      self.scene.add(new THREE.Mesh(tubeGeo, tubeMat));

      var arrowPos = path.getPoint(0.75);
      var tangent = path.getTangent(0.75).normalize();
  ...
  McpArch3D.prototype.getTypeColor = function (label) {
    if (label.startsWith('MCP Host')) return '#20B2AA';
    if (label.startsWith('Client')) return '#3b82f6';
    return '#f59e0b';
  };
